 ...
 */
/*
 * adb全称 ：Android Debug Bridge，起到调试桥的作用。是用来操作安卓设备的
 * adb是连接Android手机和pc端的桥梁，可以让用户在电脑上对手机进行操作
 */
public class AdbTool {
    ...
    public static String getLaunchableAcvivity(String apkFilePath){
        ...
        //返回值为相应apk路径下的具体包下的活动
    }

    public static boolean hasInstalledPackage(IDevice device, final String packageName) throws TimeoutException, AdbCommandRejectedException, ShellCommandUnresponsiveException, IOException {
     ...
     * @throws IOException
     */
    public static boolean startActivity(final IDevice device, final String activityName) throws TimeoutException, AdbCommandRejectedException, ShellCommandUnresponsiveException, IOException{
        final boolean[] result = new boolean[1];
        result[0] = false;
        if(device != null) {
    ...
    public static int getTaskId(IDevice device, final String taskName) throws TimeoutException, AdbCommandRejectedException, ShellCommandUnresponsiveException, IOException {
